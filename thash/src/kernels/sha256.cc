    ...
    output_stream<uint32_t> *__restrict output)
{
    ...
    /*
     * Cached SHA256 state after:
     *
     *     pub_seed[32] || zero[32]
     *
     * This state is common to all thash calls sharing pub_seed.
     */
    ...
    /*
     * ============================================================
     * Check whether pub_seed changed.
     * ============================================================
     */

    bool seed_changed = !seed_valid;
    ...
    if (!seed_changed) {
        ...
        chess_loop_range(8, 8)
        {
            if (cached_pub_seed[i] != pub_seed[i]) {
                seed_changed = true;
            }
        }
    }
     ...
     * )
     *
     * Only needed once for a fixed pub_seed.
     * ============================================================
     */
    ...
    if (seed_changed) {
        ...
        /*
         * Cache current pub_seed.
         */
        for (int i = 0; i < 8; ++i)
        chess_prepare_for_pipelining
        chess_loop_range(8, 8)
        {
            cached_pub_seed[i] = pub_seed[i];
        }
    ...
    /*
     * ============================================================
     * Start this thash from precomputed seeded state.
     * ============================================================
     */
     ...
     * ============================================================
     *
     * Construct final SHA256 block:
     *
     *     ADRS[0..21] || data[0..31] || 0x80 || 0-padding
     *
     * Total original SHA256 message length:
     *
     *     64 + 22 + 32 = 118 bytes
     *
     *     118 * 8 = 944 bits = 0x03B0
     *
     * IMPORTANT:
     * ADRS is 22 bytes, therefore data begins halfway through
     * a 32-bit word.
     *
     * Input words are assumed to preserve byte order:
     *
     *     51 d9 30 16 -> 0x51d93016
     *
     * ============================================================
     */
    ...
    /*
     * ADRS bytes 20,21 + data bytes 0,1
     */
    w[5] =
        (addr[5] & 0xFFFF0000U) |
        (data[0] >> 16);
    ...
    /*
     * Remaining data is shifted by 2 bytes because data begins
     * at byte position 22.
     */
    ...
    /*
     * Last two bytes of data followed by:
     *
     *     0x80 00
     */
    w[13] =
    ...
    /*
     * Total SHA256 bit length:
     *
     * 118 bytes * 8 = 944 = 0x3B0
     */
    w[15] = 0x000003B0U;
    ...
    /*
     * ============================================================
     * Exactly ONE SHA256 compression for this WOTS chain step.
     * ============================================================
     */
