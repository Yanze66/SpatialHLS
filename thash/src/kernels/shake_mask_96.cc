    ...
    output_stream<uint32_t> *__restrict mask_out)
{
     ...
     * Input:
     *
     *   input[0..7]   = original chain data
     *   input[8..15]  = public seed
     *   input[16..23] = original address
     *
     * SHAKE256 input:
     *
     *   PRF_PREFIX[8]
    ...
    /*
     * data[] is still required after Keccak because
     * SHAKE digest must be XORed with original data.
     */
    alignas(32) uint32_t data[8];

    /*
     * pub_seed[] must be forwarded after MASK output.
     */
    alignas(32) uint32_t pub_seed[8];

    /*
     * 25 x 64-bit Keccak state.
     *
     * Keep your original fast fused Keccak implementation.
     */
     ...
     *   0 0 0 0 0 0 0 3
     *
     * Directly initialize Keccak lanes.
     * ============================================================
     */
     ...
     * 3. pub_seed[0..7]
     *
     * Read two stream words at a time and directly build
     * one Keccak lane.
     *
     * Keep each word because pub_seed must be forwarded.
     * ============================================================
     */

    {
        const uint32_t p0 = readincr(input);
        const uint32_t p1 = readincr(input);
    ...
    {
        const uint32_t p2 = readincr(input);
        const uint32_t p3 = readincr(input);
    ...
    {
        const uint32_t p4 = readincr(input);
        const uint32_t p5 = readincr(input);
    ...
    {
        const uint32_t p6 = readincr(input);
        const uint32_t p7 = readincr(input);
     ...
     * 4. Address
     *
     * MASK does NOT need to preserve the address afterward.
     *
     * Therefore there is no addr[] array at all.
     *
     * Read stream and directly build s[8..11].
     * ============================================================
     */
     ...
     *
     * original addr7 is consumed but MASK replaces
     * it with domain value 1.
     */
    {
        const uint32_t a6 =
            readincr(input);
        ...
        s[11] =
            ...
            (static_cast<uint64_t>(0x00000001U) << 32);
    }


    /*
     * ============================================================
     * 5. SHAKE256 fixed padding
     *
    ...
    /*
     * SHAKE domain separator 0x1F at byte 96.
     */
    s[12] =
    ...
    /*
     * Last byte of the 136-byte SHAKE256 rate:
     *
     * byte 135 |= 0x80
     */
    s[16] =
     ...
     * 6. Capacity = 512 bits
     *
     * Explicit assignment avoids zeroing the whole state first.
     * ============================================================
     */
     ...
     * 8. MASK output
     *
     * SHAKE256 first 32 bytes XOR original chain data.
     *
     * Fully fixed four-lane extraction.
     * ============================================================
     */
    ...
    /*
     * ============================================================
     * 9. Forward pub_seed unchanged.
     * ============================================================
     */

    writeincr(mask_out, pub_seed[0]);
    writeincr(mask_out, pub_seed[1]);
    writeincr(mask_out, pub_seed[2]);
    writeincr(mask_out, pub_seed[3]);

    writeincr(mask_out, pub_seed[4]);
    writeincr(mask_out, pub_seed[5]);
    writeincr(mask_out, pub_seed[6]);
    writeincr(mask_out, pub_seed[7]);
}